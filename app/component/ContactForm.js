...
export default function ContactForm() {
  ...
  const mathQuiz = () => {
    fetch('https://kornberg.thebearmarketingfirm.com/wp-json/get_form/v1/contact-form/13598/')
  ...
  const submitContactForm = async (event) => {
    ...
    formData.append('f-name', event.target.fname.value);
    formData.append('f-phone', event.target.fphone.value);
    formData.append('f-email', event.target.femail.value);
    formData.append('f-message', event.target.message.value);
    ...
    await axios.post("https://kornberg.thebearmarketingfirm.com/wp-json/contact-form-7/v1/contact-forms/13598/feedback/", formData)
      .then((response) => {
  ...
  return (
    <>
      <div id="contact-form">
        <div className="wpcf7 no-js" id="wpcf7-f2656-o1" lang="en-US" dir="ltr">
          <form className={`wpcf7-form ${data?.status === "validation_failed" ? "invalid" : ""}`} noValidate="novalidate" onSubmit={submitContactForm}>
            <div className="row g-2">
              <div className="col-md-6">
                <span className="wpcf7-form-control-wrap">
                  ...
                  {data?.invalid_fields?.find((field) => field.field === "f-name")?.message && (<span className="wpcf7-not-valid-tip">{data.invalid_fields.find((field) => field.field === "f-name").message}</span>)}
                </span>
                <span className="wpcf7-form-control-wrap">
                  <input className="form-control" placeholder="Phone*" type="text" id="phone" name="fphone" onChange={handlePhoneChange} required />
                  {data?.invalid_fields?.find((field) => field.field === "f-phone")?.message && (<span className="wpcf7-not-valid-tip">{data.invalid_fields.find((field) => field.field === "f-phone").message}</span>)}
                </span>
                <span className="wpcf7-form-control-wrap">
                  <input className="form-control" placeholder="Email*" type="email" id="femail" name="femail" required />
                  {data?.invalid_fields?.find((field) => field.field === "f-email")?.message && (<span className="wpcf7-not-valid-tip">{data.invalid_fields.find((field) => field.field === "f-email").message}</span>)}
                </span>
              </div>
              <div className="col-md-6">
                <span className="wpcf7-form-control-wrap">
            ...
            <p className="form-disc-policy">By clicking submit, you are agreeing to the <Link href="/disclaimer/" title="Disclaimer">Disclaimer</Link> and <Link href="/privacy-policy/" title="Privacy Policy">Privacy Policy</Link></p>
