...
export async function generateMetadata(posttype, slug) {
  const meta = await MetaData(posttype = "pages", slug = "privacy-policy");
  if (meta) { return meta; }
...
export default async function PracticeAreas() {
  const [[pageApi]] = await Promise.all([
    fetch('https://kornberglawfirm.com/wp-json/wp/v2/pages/?slug=privacy-policy&_fields=link,title,content', {next:{revalidate:3600 }}).then(res => res.json()),
  ]);
  return (
    <>
