...
export async function generateMetadata(posttype, slug) {
  const meta = await MetaData(posttype = "pages", slug = "thank-you");
  if (meta) { return meta; }
...
export default async function ThankYou() {
  const [[ThankYouData]] = await Promise.all([
    fetch('https://kornberglawfirm.com/wp-json/wp/v2/pages/?slug=thank-you&_fields=title,content', {next:{revalidate:3600 }}).then(res => res.json()),
  ]);
